...
export class Raycaster {
    ...
    drawWalls(ctx){
        ...
        for (let i=0; i < this.rays.length; i++) {
            let curcol = this.game.width - this.rays.length + i; 
            ...
            if (this.rays[i].side == 0) ctx.strokeStyle = '#448101';
            else ctx.strokeStyle = '#55a002';
            ctx.beginPath();
            ctx.moveTo(curcol,drawStart);
            ctx.lineTo(curcol, drawEnd);
            ctx.stroke();
            ctx.closePath();
            //floor and ceiling 
            ctx.beginPath();
            ctx.strokeStyle = '#555555';
            ctx.moveTo(curcol, drawEnd);
            ctx.lineTo(curcol, this.game.height);
            ctx.stroke();
            ctx.closePath();
            ctx.beginPath();
            ctx.strokeStyle = '#999999';
            ctx.moveTo(curcol,0);
            ctx.lineTo(curcol,drawStart);
            ctx.stroke();
            ctx.closePath();
        }
    }
